...
make_file_from_dvd() {
  ...
  [ "$type" = "DVD" ] || exec echo "$dvdrom is not a DVD." >/dev/stderr;
  ...
  runx HandBrakeCLI --decomb --format mkv --markers --two-pass --turbo --quality 23 \
    --encoder x264 --x264-preset slow --x264-tune film --x264-profile main \
    --input "$dvdrom" --output "$video_file";
}
